...
public class AllTypesServiceImpl implements AllTypesService {
    ...
    @Transactional(readOnly = true, value = "dbscenariosTransactionManager")
    @Override
    public AllTypes getByFloatCol(Float floatCol) {
        Map<String, Object> floatColMap = new HashMap<>();
        floatColMap.put("floatCol", floatCol);

        LOGGER.debug("Finding AllTypes by unique keys: {}", floatColMap);
        AllTypes allTypes = this.wmGenericDao.findByUniqueKey(floatColMap);

        if (allTypes == null){
            LOGGER.debug("No AllTypes found with given unique key values: {}", floatColMap);
            throw new EntityNotFoundException(String.valueOf(floatColMap));
        }
